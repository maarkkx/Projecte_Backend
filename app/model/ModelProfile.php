<?php
function profile_user_exists(string $user, $conn): bool {
    $stmt = $conn->prepare("SELECT 1 FROM users WHERE user = :u LIMIT 1");
    $stmt->execute([':u' => $user]);
    return (bool)$stmt->fetchColumn();
}

function profile_update_username(string $oldUser, string $newUser, $conn): bool {
...
function getPasswd(string $user, $conn): ?string {
 ...
 */
function updPasswd(string $user, string $newHash, $conn): bool {
