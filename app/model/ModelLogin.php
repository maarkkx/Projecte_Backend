...
function crearUsuari($user, $nombre, $apellido, $correo, $password) {
    global $conn;
    try {
        $sql = "INSERT INTO users (user, nombre, apellido, correo, password) VALUES (:user, :nombre, :apellido, :correo, :password)";
...
function loginUsuari($user, $password) {
    ...
    try {
        ...
        if ($result) {
            return "Log In correcte";
        } else {
            return "Usuari o contrasenya incorrecte";
        }
